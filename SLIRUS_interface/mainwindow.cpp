...
void MainWindow::SendStrToOpenCR() //Function sends string to openCR and plots the right character in interface
{
    ...
    if (trad_string != NULL)
    {
        ...
        connect(process, &QProcess::readyReadStandardOutput, [process, this]()
        {
            ...
            if (label.startsWith("Interface:next", chartemp) == true)
            {
            ...
            else if (label.startsWith("Interface:stop", chartemp) == true)
            {
                Letter_increment = 1;
        ...
        connect(process, &QProcess::readChannelFinished, [process, this]()->void
        {
                qDebug()<< "in writing" << endl;
                process->write((QString("j\n")).toLatin1());
...
void MainWindow::on_transcript_charac_returnPressed()
{
    ui->Start_record_button->setEnabled(false);
    ui->See_charac->setEnabled(false);
    ui->Recording_time->setEnabled(false);
